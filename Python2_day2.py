...
def gratuitous_arp(ip_address, ifname='ens37'):
    ...
    gratuitous_arp_pkt = kamene.all.Ether(src=localmac, dst='ff:ff:ff:ff:ff:ff') / kamene.all.ARP(op=2,
                                                                                                  hwsrc=localmac,
                                                                                                  hwdst=localmac,
                                                                                                  psrc=ip_address,
                                                                                                  pdst=ip_address)

    kamene.all.sendp(gratuitous_arp_pkt, iface=scapy_iface(ifname), verbose=False)
